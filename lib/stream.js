define(['./request',
        ...
        'class',
        'debug'],
function(Request, utils, BOSHError, xml, ajax, Emitter, clazz, debug) {
  ...
  Stream.prototype.pause = function(sec) {
    ...
    if (!this.maxpause) throw new Error('BOSH connection manager does not support session pausing')
  ...
  Stream.prototype._enqueue = function(xml, options, cb) {
    var isBody = false;
    ...
    this._queue.push({ payload: xml, attrs: options, fn: cb, isBody: isBody });
